...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, GUID codecGuid, GUID presetGuid)
{
    ...
    pIntializeParams->encodeConfig->rcParams.maxBitRate = 5000000;
	pIntializeParams->encodeConfig->rcParams.averageBitRate = 6000000;
    ...
    pIntializeParams->encodeConfig->gopLength = 60;// NVENC_INFINITE_GOPLENGTH;

    pIntializeParams->encodeConfig->rcParams.rateControlMode = NV_ENC_PARAMS_RC_CBR_LOWDELAY_HQ;// NV_ENC_PARAMS_RC_CBR_HQ;// NV_ENC_PARAMS_RC_CONSTQP;
        ...
        && pIntializeParams->presetGUID != NV_ENC_PRESET_LOSSLESS_HP_GUID)
    {
        ...
        pIntializeParams->encodeConfig->rcParams.maxQP = { 32, 36, 32 };
    }
...
void NvEncoder::CreateEncoder(const NV_ENC_INITIALIZE_PARAMS* pEncoderParams)
{
    ...
    else
    {
        ...
        m_encodeConfig.version = NV_ENC_CONFIG_VER;
        m_encodeConfig.rcParams.rateControlMode = NV_ENC_PARAMS_RC_CBR_HQ;// NV_ENC_PARAMS_RC_CONSTQP;
...
void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<std::vector<uint8_t>> &vPacket, bool bOutputDelay)
{
	...
	//std::cout << "nvEncEncodePicture used time : " << duration << std::endl;

    unsigned i = 0;
    int iEnd = bOutputDelay ? m_iToSend - m_nOutputDelay : m_iToSend;
    //std::cout << "m_iGot : " << m_iGot << " iEnd : " << iEnd << std::endl;
    for (; m_iGot < iEnd; m_iGot++)
    {
		...
		auto en_end = std::chrono::high_resolution_clock::now();
		int duration = (int)std::chrono::duration<double, std::milli>(en_end - en_beg).count();
		//std::cout << "nvEncLockBitstream used time : " << duration << " i : " << m_iGot << std::endl;
