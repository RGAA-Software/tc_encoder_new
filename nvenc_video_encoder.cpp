...
void NVENCVideoEncoder::FillEncodeConfig(NV_ENC_INITIALIZE_PARAMS& initializeParams, int refreshRate, int renderWidth, int renderHeight, uint64_t bitrate_bps)
{
    auto& encodeConfig = *initializeParams.encodeConfig;

    GUID encoderGUID = encoder_config_.codec_type == tc::EVideoCodecType::kH264 ? NV_ENC_CODEC_H264_GUID : NV_ENC_CODEC_HEVC_GUID;
